...
#Define A Star
def path_planning(nStart,nGoal,robot_radius,grid,ox,oy,algo_flag,res,show_animation,flag):
            ...
            if new_id in active:
                if active[new_id].c2c > new_node.c2c:
                    ...
                    if algo_flag is "astar":
                        heapq.heappush(cost_heap,[active[new_id].c2c+calc_heuristic(nGoal,active[new_id],flag),active[new_id]])
...
def main():
    ...
    Parser.add_argument('--hur', default="euc", help='Heuristic function for A-star: man- Manhattan and euc- Euclidean')
    ...
    if start==True and goal==True:
        start = time.time()
        rx,ry,dead = path_planning(nStart,nGoal,robot_radius=robot_radius,grid=grid,ox=ox,oy=oy,algo_flag=algo_flag,res=res,show_animation=show_animation,flag = flag)
        end = time.time()
